    ...
    <div class="min-h-screen bg-gray-50 p-6 border border-gray-200 rounded-xl flex flex-col gap-2">
        ...
        <div>
            <a href="{{ route('admin.manage-user.index') }}" class="text-sm text-gray-600 hover:underline">/Kelola
                Pengguna</a><span class="text-sm text-gray-800 font-semibold">/Formulir Tambah Data Pengguna</span>
        </div>
        <form action="{{ route('admin.add-user.store') }}" method="POST" class="mt-6 bg-white p-6 rounded-lg shadow">
            @csrf
            <div class="space-y-4">
                <div>

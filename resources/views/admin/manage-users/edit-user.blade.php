...
@section('container')
    <div class="min-h-screen bg-gray-50 p-6 border border-gray-200 rounded-xl flex flex-col gap-2">
        <h1 class="text-2xl font-semibold text-gray-800">Edit Data Pengguna</h1>
        <div>
            <a href="{{ route('admin.manage-user.index') }}" class="text-sm text-gray-600 hover:underline">/Kelola
                Pengguna</a><span class="text-sm text-gray-800 font-semibold">/Formulir Edit Data Pengguna</span>
        </div>
        <form action="{{ route('admin.edit-user.update', $user->id) }}" method="POST" class="mt-6 bg-white p-6 rounded-lg shadow">
            @csrf
            @method('PUT')
            <div class="space-y-4">
                <div>
                ...
                <div>
                    ...
                    <select id="role" name="role" required value="{{ old('role', $user->role) }}"
